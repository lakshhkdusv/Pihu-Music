...
from NETFLIXMUSIC.utils.Shukla_ban import admin_filter
from NETFLIXMUSIC.mongo.notesdb import *
from NETFLIXMUSIC.utils.notes_func import GetNoteMessage, exceNoteMessageSender, privateNote_and_admin_checker
from NETFLIXMUSIC.utils.yumidb import user_admin
...
async def PrivateNote(client, message):
    ...
    if len(message.command) >= 2:
        ...
        if await is_pnote_on(chat_id):
            await message.reply(
                "Your notes are currently being sent in private. NETFLIXMUSIC will send a small note with a button which redirects to a private chat.",
                quote=True
            )
        else:
            await message.reply(
